...
exports.sh = sh;
exports.tokenEnv = tokenEnv;
exports.haveGh = haveGh;
exports.ghAuthOk = ghAuthOk;
exports.ghRaw = ghRaw;
exports.ghApi = ghApi;
exports.ghApiJson = ghApiJson;
exports.ghPutJson = ghPutJson;
exports.openBrowser = openBrowser;
const child_process_1 = require("child_process");
function sh(cmd, args, extraEnv, input) {
    const env = extraEnv ? { ...process.env, ...extraEnv } : process.env;
    const r = (0, child_process_1.spawnSync)(cmd, args, { encoding: "utf8", env, input });
...
function ghRaw(args, token) {
    ...
    if (!r.ok)
        throw new Error(`gh ${args[0]} failed [exit ${r.code}]: ${r.err || r.out || "(no output)"}`);
    return r.out;
}
function ghApi(token, endpoint, fields, method) {
    ...
    if (!r.ok)
        throw new Error(`GitHub API ${method || "GET"} ${endpoint} failed [exit ${r.code}]: ${r.err || r.out || "(no output)"}`);
    return r.out;
}
...
function ghPutJson(token, endpoint, obj) {
    ...
    if (!r.ok)
        throw new Error(`GitHub API PUT ${endpoint} failed [exit ${r.code}]: ${r.err || r.out || "(no output)"}`);
    return r.out ? JSON.parse(r.out) : null;
}
function openBrowser(url) {
    const plat = process.platform;
    if (plat === "darwin")
        return sh("open", [url]).ok;
    if (plat === "win32")
        return sh("cmd", ["/c", "start", "", url]).ok;
